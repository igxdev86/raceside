    ...
    (cards.racecards || []).forEach((rc) => {
      ...
      priced.forEach((p, i) => {
        rides.push({
          ...
          h: p.x.horse || '', num: (() => { const v = String(p.x.number == null ? '' : p.x.number).trim(); return v && v.toUpperCase() !== 'NR' ? v : null; })(), hid: p.x.horse_id || null, own: p.x.owner || null, oid: p.x.owner_id || null, d: Math.round(p.d * 100) / 100,
