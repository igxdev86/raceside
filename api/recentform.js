...
export default async function handler(req, res) {
  ...
  const J = {}, T = {};
  const touch = (bucket, id, won, date) => {
    const s = (bucket[id] ||= { r: 0, w: 0, since: 0, lastWin: null });
    s.r++;
  ...
  for (const race of races) {
    for (const x of race.runners || []) {
      ...
      if (x.jockey_id) touch(J, x.jockey_id, won, race.date);
      if (x.trainer_id) touch(T, x.trainer_id, won, race.date);
    }
  }
  res.setHeader('Cache-Control', 's-maxage=10800, stale-while-revalidate=21600');
  return res.status(200).json({ ok: true, window: 21, from: fmt(start), to: fmt(end), jockeys: J, trainers: T });
}
